...
# ------------------------- data generation ---------------------------
def generate_sizes(N, alpha, rng, dist="pareto"):
    """Firm sizes under alternative generative families.
    'pareto'    : power-law; smaller alpha -> heavier tail -> more concentrated.
    'lognormal' : sigma tied to 1/alpha so smaller alpha -> wider spread.
    'truncated' : Pareto truncated at an upper bound (thinner extreme tail).
    """
    if dist == "pareto":
        return rng.pareto(alpha, size=N) + 1.0
    if dist == "lognormal":
        sigma = 1.0 / alpha  # smaller alpha -> larger sigma -> more concentrated
        return rng.lognormal(mean=0.0, sigma=sigma, size=N)
    if dist == "truncated":
        x = rng.pareto(alpha, size=N) + 1.0
        cap = np.quantile(x, 0.95)  # truncate the extreme upper tail
        return np.minimum(x, cap)
    raise ValueError(f"unknown size distribution: {dist}")
...
def generate_scenario(cfg, scenario_name):
    ...
    g = cfg["g_function"]
    size_dist = cfg.get("size_distribution", "pareto")

    rows = []
    for dom in DOMAINS:
        N, alpha = sc["N"][dom], sc["alpha"][dom]
        size = generate_sizes(N, alpha, rng_struct, dist=size_dist)
        share = size / size.sum()
        ...
        beta_form = cfg.get("beta_form", "linear")
        eps = rng_beta.normal(0, g["sigma_beta"], size=N)
        if beta_form == "linear":
            # baseline: additive logit (matches b's linear aggregation)
            logit_beta = (
        ...
        elif beta_form == "multiplicative":
            # Cobb-Douglas-like: power emerges from the PRODUCT of drivers,
            # functionally disjoint from b's weighted sum.
            size01 = sigmoid(g["theta_size"] * log_size_c)
            m = (size01 ** 0.5) * (irr ** 0.8) * ((0.5 + conc) ** 0.4)
            beta_star = np.clip(m * np.exp(0.3 * eps), 0, 1)
        elif beta_form == "min":
            # Leontief-like: power is bottlenecked by the WEAKEST driver
            # (a min), strongly non-linear vs. a weighted sum.
            size01 = sigmoid(g["theta_size"] * log_size_c)
            conc_arr = np.full(N, np.clip(conc + 0.3, 0, 1))
            m = np.minimum(np.minimum(size01, irr), conc_arr)
            beta_star = np.clip(m + 0.1 * eps, 0, 1)
        else:
            raise ValueError(f"unknown beta_form: {beta_form}")
