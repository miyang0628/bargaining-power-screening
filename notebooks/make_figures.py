...
def fig5_baseline():
    """Revision: competence-only vs two-axis — super-supplier retention and mean beta*."""
    tab = pd.read_csv(ROOT / "results" / "tables" / "table_baseline.csv")
    order = ["oligopolistic", "intermediate", "fragmented"]
    tab = tab.set_index("scenario").reindex(order).reset_index()
    labels = [s.capitalize() for s in tab["scenario"]]
    x = np.arange(len(labels)); w = 0.36
    fig, axes = plt.subplots(1, 2, figsize=(7.4, 3.2))

    axes[0].bar(x - w/2, tab["comp_super_retention"], w, label="Competence-only",
                color="0.30", edgecolor="k", linewidth=0.6)
    axes[0].bar(x + w/2, tab["two_super_retention"], w, label="Two-axis",
                color="0.70", edgecolor="k", linewidth=0.6)
    axes[0].set_ylim(0, 1.05)
    axes[0].set_ylabel("Super-supplier retention rate")
    axes[0].set_xticks(x); axes[0].set_xticklabels(labels, fontsize=8)
    axes[0].legend(fontsize=7)

    axes[1].bar(x - w/2, tab["comp_max_beta"], w, label="Competence-only",
                color="0.30", edgecolor="k", linewidth=0.6)
    axes[1].bar(x + w/2, tab["two_max_beta"], w, label="Two-axis",
                color="0.70", edgecolor="k", linewidth=0.6)
    axes[1].set_ylabel(r"Max $\beta^{*}$ in selected set")
...
def fig6_betaform_simplex():
    """Revision: (a) convergent corr under disjoint beta* forms; (b) AUC distribution over weight simplex."""
    bf = pd.read_csv(ROOT / "results" / "tables" / "table_beta_form.csv")
    sx = pd.read_csv(DATA / "weight_simplex.csv")
    order = ["oligopolistic", "intermediate", "fragmented"]
    forms = ["linear", "multiplicative", "min"]
    fig, axes = plt.subplots(1, 2, figsize=(7.4, 3.2))

    # (a) grouped bars: convergent corr by beta-form
    x = np.arange(len(forms)); w = 0.25
    shades = ["0.25", "0.5", "0.72"]
    for k, scen in enumerate(order):
        vals = [bf[(bf.beta_form == f) & (bf.scenario == scen)]["corr_b_beta"].values[0] for f in forms]
        axes[0].bar(x + (k-1)*w, vals, w, label=scen.capitalize(),
                    color=shades[k], edgecolor="k", linewidth=0.6)
    axes[0].set_xticks(x); axes[0].set_xticklabels([f.capitalize() for f in forms], fontsize=8)
    axes[0].set_ylabel(r"Convergent corr($b,\beta^{*}$)")
    axes[0].set_ylim(0, 1.0); axes[0].legend(fontsize=6.5)
    axes[0].set_xlabel(r"$\beta^{*}$ functional form")

    # (b) AUC distribution over weight simplex (boxplot by scenario)
    data = [sx[sx.scenario == s]["auc"].values for s in order]
    bp = axes[1].boxplot(data, labels=[s.capitalize() for s in order],
                         patch_artist=True, widths=0.5)
    for patch in bp["boxes"]:
        patch.set_facecolor("0.75"); patch.set_edgecolor("k")
    for med in bp["medians"]:
        med.set_color("k")
    axes[1].set_ylabel("Discrimination AUC")
    axes[1].set_xlabel("Weight-simplex draws (n=300 each)")
    axes[1].tick_params(axis="x", labelsize=8)
    fig.tight_layout()
    _save(fig, "fig6_betaform_simplex")
